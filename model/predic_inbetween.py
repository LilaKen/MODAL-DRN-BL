...
from stressnet.Inbetween import Inbetween
from inital.import_dataset import data_main

def parse_args():
    parser = argparse.ArgumentParser(description='Training and testing script for Inbetween model.')
...
def run_training(args, train_dataset, val_dataset):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        checkpoint_path = '../output/inbetween/checkpoint_latest_inbetween.pth.tar'
        save_checkpoint({
            'epoch': epoch + 1,
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        remaining_data = torch.concat([keypoint_rows, line_rows, remaining_data], dim=2)
        x_shape = remaining_data
        x_load = load_rows
        x_boundary = dof_rows

        output = model(x_shape, x_load, x_boundary)
...
def save_checkpoint(state, is_best, filename):
    ...
    if is_best:
        shutil.copyfile(filename,  '../output/inbetween/model_inbetween_best.pth.tar')
