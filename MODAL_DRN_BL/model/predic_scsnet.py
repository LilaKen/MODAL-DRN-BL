...
from stressnet.SCSNet import SCSNet
from inital.import_dataset import data_main

def parse_args():
    parser = argparse.ArgumentParser(description='Training and testing script for SCSNet model.')
...
def run_training(args, train_dataset, val_dataset):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        checkpoint_path = '../output/scsnet/checkpoint_latest_scsnet.pth.tar'
        save_checkpoint({
            'epoch': epoch + 1,
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        removed_rows = input_var[:, :, :8, :]
        remaining_data = input_var[:, :, 8:, :]
        ...
        remaining_data[:, :, :2, :] += keypoint_rows
        remaining_data[:, :, 2:3, :] += line_rows
        remaining_data[:, :, 3:7, :] += dof_rows

        input_var = remaining_data

        output = model(input_var, load_rows)
...
def save_checkpoint(state, is_best, filename):
    ...
    if is_best:
        shutil.copyfile(filename,  '../output/scsnet/model_scsnet_best.pth.tar')
